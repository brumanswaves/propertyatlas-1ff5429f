...
begin
  ...
  case p_action
    ...
    when 'reopen_review' then
      if v_from_status not in ('ready', 'failed') then
        raise exception 'Only delivered or failed orders can be reopened for review';
      end if;
      v_to_status := 'processing';
      update public.report_orders
      set status = 'processing',
...
is 'Applies audited Done-for-You fulfillment and permits an explicitly failed or delivered investigation to be reopened safely into processing.';
