...
const FIELDS: { key: keyof Omit<NoteRow, "checklist">; label: string; placeholder: string; rows?: number }[] = [
  { key: "personal",      label: "Personal notes",      placeholder: "First impressions, ideas, plans…", rows: 3 },
  ...
  { key: "questions",     label: "Questions to verify", placeholder: "What you need to confirm before deciding" },
  { key: "agent_contact", label: "Agent contact",       placeholder: "Name, agency, phone, email" },
  { key: "municipality",  label: "Municipality notes",  placeholder: "Rates, zoning queries, planning enquiries" },
  { key: "renovation",    label: "Renovation notes",    placeholder: "Scope, quotes, contractors" },
];

const EMPTY: NoteRow = {
  personal: "", pros: "", cons: "", questions: "",
  agent_contact: "", municipality: "", renovation: "",
  checklist: {},
};

export function NotesTab({ parcelId }: { parcelId: string }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    supabase.from("property_notes").select("*").eq("parcel_id", parcelId).maybeSingle()
      .then(({ data }) => {
        if (data) setRow({ ...EMPTY, ...data, checklist: (data.checklist as any) ?? {} });
        setLoaded(true);
      });
  }, [user, parcelId]);

  async function save() {
    if (!user) { toast.message("Sign in to save notes"); return; }
    setSaving(true);
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">Your research is private and only visible to you.</p>
        </div>
      ...
      <section>
        <div className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Due diligence checklist</div>
              ...
              <li key={item.id}>
                <button
                  ...
                  className={
                    ...
                    (checked ? "bg-emerald-50 text-emerald-900 dark:bg-emerald-900/20 dark:text-emerald-200" : "hover:bg-muted")
                  }
                >
                  {checked ? <CheckSquare className="h-3.5 w-3.5 text-emerald-600" /> : <Square className="h-3.5 w-3.5 text-muted-foreground" />}
      ...
      <ComplianceNotice tone="soft">
        Notes are stored for your account only. Do not paste copyrighted listing text or proprietary report content.
      </ComplianceNotice>
