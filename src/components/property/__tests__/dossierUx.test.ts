...
describe("official dossier UX guardrails", () => {
  ...
  it("renders the listings and comps tab and BRRRR explanation", () => {
    ...
    expect(strategyLab).toContain("Guided Investigation / Strategy");
    expect(strategyLab).toContain("1. Choose a strategy");
    expect(strategyLab).toContain("2. Review assumptions");
    expect(strategyLab).toContain("3. Review the result");
    expect(strategyLab).toContain("Advanced assumptions");
...
describe("Local Property Team MVP guardrails", () => {
  it("connects the report to a real Local Property Team and hides purchases", () => {
    ...
    expect(serverRoute).toContain("GOOGLE_CANDIDATE_PAGE_SIZE = 10");
    expect(serverRoute).toContain("pageSize: GOOGLE_CANDIDATE_PAGE_SIZE");
    ...
    expect(serverRoute).not.toContain("CACHE_TTL_MS");
    expect(serverRoute).not.toContain("__easyErfLocalServicesCache");
