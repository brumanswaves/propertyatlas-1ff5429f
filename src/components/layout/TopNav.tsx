  ...
  return (
    <header className="absolute inset-x-0 top-0 z-30 flex items-center justify-between gap-2 px-4 py-3 md:px-6">
      ...
      <nav className="hidden items-center gap-1 rounded-full bg-card/90 p-1.5 shadow-soft backdrop-blur md:flex">
        ...
        {user ? (
          <>
            <Link to="/dashboard" className="px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground">
              Dashboard
            </Link>
            <Button
              size="sm"
      ...
      {open && (
        <>
          <div onClick={() => setOpen(false)} className="fixed inset-0 z-40 bg-foreground/30 backdrop-blur-sm md:hidden" />
          <div className="fixed inset-x-3 top-16 z-50 max-h-[80vh] overflow-y-auto rounded-2xl border border-border bg-card p-3 shadow-panel md:hidden">
            ...
            <div className="mt-2 flex items-center gap-2 border-t border-border pt-3">
              {user ? (
                <>
                  <Link to="/dashboard" onClick={() => setOpen(false)} className="flex-1 rounded-lg border border-border px-3 py-2 text-center text-xs font-medium">Dashboard</Link>
                  <Button size="sm" variant="ghost" className="h-9 flex-1 rounded-lg text-xs" onClick={() => supabase.auth.signOut()}>Sign out</Button>
                </>
              ) : (
                <>
                  <Link to="/auth" onClick={() => setOpen(false)} className="flex-1 rounded-lg border border-border px-3 py-2 text-center text-xs font-medium">Sign in</Link>
                  <Link to="/auth" onClick={() => setOpen(false)} className="flex-1 rounded-lg bg-gradient-brand px-3 py-2 text-center text-xs font-semibold text-primary-foreground">
                    Start free
                  </Link>
                </>
              )}
