...
export function TopNav({ center, mobileCenter, onLogoClick, subtitle }: TopNavProps = {}) {
  ...
  const navLinkBase =
    "relative rounded-lg px-3 py-1.5 text-[12px] font-semibold text-white/80 transition hover:bg-white/[0.06] hover:text-white";
  const navLinkActive =
    "bg-white/[0.06] text-white after:absolute after:inset-x-2 after:-bottom-1 after:h-[2px] after:rounded-full after:bg-accent";

  return (
    <header
      className={
        mapHeader
          ? "fixed inset-x-0 top-0 z-[70] border-b border-white/10 bg-primary/95 px-3 pb-2 pt-[calc(env(safe-area-inset-top)+0.5rem)] shadow-panel backdrop-blur-xl md:px-6 md:pt-2"
          : "absolute inset-x-0 top-0 z-30 flex items-center justify-between gap-2 border-b border-white/10 bg-primary/95 px-4 py-3 shadow-sm backdrop-blur-xl md:px-6"
      }
    >
      <div
        className={
      ...
      >
        <Link
          to="/"
          onClick={onLogoClick}
          className="group inline-flex h-10 w-[148px] shrink-0 items-center justify-center rounded-full border border-white/20 bg-white px-4 shadow-soft ring-1 ring-white/10 transition hover:bg-white/95 hover:shadow-panel md:w-[156px]"
          ...
          {user ? (
            <>
              ...
              <Button
                size="sm"
                variant="ghost"
                className="ml-1 h-8 rounded-full border border-white/20 bg-white/[0.06] text-[12px] font-semibold text-white hover:bg-white/12 hover:text-white"
        ...
        <button
          ...
          className="grid h-10 w-10 place-items-center rounded-full bg-white/[0.08] text-white ring-1 ring-white/20 backdrop-blur md:hidden"
      ...
      {mapHeader && (
        <div className="mt-2 grid gap-1.5 md:hidden">
          {mobileCenter}
          {subtitle && (
            <div className="hidden rounded-xl bg-white/[0.06] px-3 py-1.5 text-center text-[11px] font-medium text-white/80 ring-1 ring-white/10 sm:block">
              {subtitle}
            </div>
          )}
        </div>
      ...
      {mapHeader && subtitle && (
        <div className="mx-auto mt-1.5 hidden max-w-3xl text-center text-[12px] font-medium text-white/75 md:block">
          {subtitle}
        </div>
      )}
