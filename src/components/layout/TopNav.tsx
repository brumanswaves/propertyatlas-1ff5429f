...
export function TopNav({ center, mobileCenter, onLogoClick, subtitle }: TopNavProps = {}) {
  ...
  const navLinkBase = mapHeader
    ? "relative rounded-lg px-3 py-1.5 text-[12px] font-semibold text-primary-foreground/80 transition hover:text-primary-foreground"
    : "relative rounded-lg px-3 py-1.5 text-[12px] font-semibold text-foreground/70 transition hover:bg-muted hover:text-foreground";
    ...
    <header
      className={
        mapHeader
          ...
          : "absolute inset-x-0 top-0 z-30 flex items-center justify-between gap-2 border-b border-border/70 bg-background/95 px-4 py-3 shadow-sm backdrop-blur-xl md:px-6"
      }
    >
      <div
        className={
      ...
      >
        <Link
          to="/"
          onClick={onLogoClick}
          className="group inline-flex h-11 w-[158px] shrink-0 items-center justify-center rounded-full border border-border/70 bg-card px-4 shadow-soft ring-1 ring-primary/5 transition hover:bg-background hover:shadow-panel md:w-[166px]"
          aria-label="Easy Erf home"
        >
          <AtlasPin
            variant="horizontal"
            className="block h-[27px] w-auto max-w-[136px] object-contain md:h-[29px] md:max-w-[142px]"
            title={BRAND.site}
          />
        </Link>
              ...
              <Button
                ...
                className={
                  mapHeader
                    ...
                    : "ml-1 h-8 rounded-full border border-border bg-card text-[12px] font-semibold text-foreground hover:bg-muted hover:text-foreground"
                }
        ...
        <button
          ...
          className={
            mapHeader
              ? "grid h-10 w-10 place-items-center rounded-full bg-primary-foreground/[0.08] text-primary-foreground ring-1 ring-primary-foreground/20 backdrop-blur md:hidden"
              : "grid h-10 w-10 place-items-center rounded-full bg-muted text-foreground ring-1 ring-border md:hidden"
          }
      ...
      {mapHeader && (
        <div className="mt-2 grid gap-1.5 md:hidden">
          {mobileCenter}
          {subtitle && (
            <div className="hidden rounded-xl bg-primary-foreground/[0.05] px-3 py-1.5 text-center text-[11px] font-medium text-primary-foreground/80 ring-1 ring-primary-foreground/10 sm:block">
              {subtitle}
            </div>
          )}
        </div>
      ...
      {mapHeader && subtitle && (
        <div className="mx-auto mt-1.5 hidden max-w-3xl text-center text-[12px] font-medium text-primary-foreground/78 md:block">
          {subtitle}
        </div>
      )}
