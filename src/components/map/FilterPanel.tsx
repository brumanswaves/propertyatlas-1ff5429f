...
const TYPES: PropertyType[] = ["Residential", "Commercial", "Industrial", "Agricultural", "Vacant Land"];
      ...
      {open && (
        <div className="absolute left-0 top-full z-40 mt-2 w-72 rounded-2xl border border-border bg-card p-4 shadow-panel">
          ...
          <Section title="Location">
            <Chip
              active={value.beachfrontOnly}
              onClick={() => onChange({ ...value, beachfrontOnly: !value.beachfrontOnly })}
            >
              Beachfront only
            </Chip>
          </Section>
          <Section title="Ownership">
            <div className="flex flex-wrap gap-1.5">
              {OWNERS.map((o) => (
                <Chip key={o} active={value.ownership.includes(o)} onClick={() => toggleOwner(o)}>
                  {o}
                </Chip>
              ))}
            </div>
