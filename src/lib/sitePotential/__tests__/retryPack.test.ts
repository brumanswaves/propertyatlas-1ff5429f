...
function createServiceClient(rows: ReturnType<typeof baseRows>, options: { beforeItemUpdate?: () => void } = {}) {
  const calls: string[] = [];
...
async function retry(rows: ReturnType<typeof baseRows>, options?: { beforeItemUpdate?: () => void }) {
